...
class Gateway {
    ...
    listUsers(authCorpSuiteAccessToken, departmentID, fetchChild = 1, status = 1, traceContext, callback) {
        var url = `https://${WECHAT_QYAPI_SERVICE_HOST}/cgi-bin/user/list?access_token=${authCorpSuiteAccessToken}&department_id=${departmentID}&fetch_child=${fetchChild}&status=${status}`;
        var options = {
            method: "GET",
            path: url
        };
