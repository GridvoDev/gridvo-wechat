...
class Service {
    ...
    getLatestSuiteAccessToken(suiteID, traceContext, callback) {
        ...
        function getSuiteAccessTokenFromWeChatServer(suiteSecret, suiteTicket) {
            return new Promise((resolve, reject)=> {
        ...
        function* getSuiteAccessToken() {
            ...
            let resultJSON = yield getSuiteAccessTokenFromWeChatServer(suiteSecret, suiteTicket);
            if (!resultJSON || !resultJSON.suite_access_token) {
                return null;
            }
            let accessToken = resultJSON.suite_access_token;
            let expire = new Date((new Date()).getTime() + 7190000);
